 ...
 *
 * Components can be layed out at their preferred size or at a relative size. When relative sizing
 * is used the component must be added to the container using a relative size constraint, which is
 * simply a Float value.
 *
 * The space available for relative sized components is determined by subtracting the preferred size
 * of the other components from the space available in the container. Each component is then
 * assigned a size based on its relative size value. For example:
 *
 * container.add(component1, new Float(1)); container.add(component2, new Float(2));
 *
 * There is a total of 3 relative units. If the container has 300 pixels of space available then
 * component1 will get 100 and component2, 200.
 *
 * It is possible that rounding errors will occur in which case you can specify a rounding policy to
 * use to allocate the extra pixels.
 *
 * By defaults components are center aligned on the secondary axis however this can be changed at
 * the container or component level.
 */
public class RelativeLayout implements LayoutManager2, java.io.Serializable {
  ...
  /**
   * The alignment of the Components on the other axis of the Container. For X-AXIS this would refer
   * to the Y alignemt. For Y-AXIS this would refer to the X alignment.
   */
  private float alignment = CENTER;

  /**
   * This is the gap (in pixels) which specifies the space between components It can be changed at
   * any time and should be a non-negative integer.
   */
  private int gap;

  /**
   * The gap (in pixels) used before the leading component and after the trailing component. It can
   * be changed at any time and should be a non-negative integer.
   */
  ...
  /**
   * Creates a relative layout with the components layed out on the specified axis using the default
   * gap
   * <p>
   * 
  ...
  /**
   * Creates a relative layout with the components layed out on the specified axis using the
   * specfied gap
   * <p>
  ...
  /**
   * Gets the initial gap. This gap is used before the leading component and after the trailing
   * component.
   *
  ...
  /**
   * Sets the initial gap. This gap is used before the leading component and after the trailing
   * component. The default is set to the gap.
   *
   ...
   *
   * For X-AXIS this would refer to the Y alignemt. For Y-AXIS this would refer to the X alignment.
   *
   * Must be between 0.0 and 1.0, or -1. Values can be specified using:
   *
   * RelativeLayout.LEADING RelativeLayout.CENTER RelativeLayout.TRAILING RelativeLayout.COMPONENT -
   * the getAlignemntX/Y method for the opposite axis will be used
   */
  ...
  /**
   * Change size of relative components to fill the space available For X-AXIS aligned components
   * the height will be filled. For Y-AXIS aligned components the width will be filled.
   */
  ...
  /**
   * Specify the number of pixels by which the fill size is decreased when setFill(true) has been
   * specified.
   */
  ...
  /**
   * Specify the rounding policy to be used when all the avialable pixels have not been allocated to
   * a component.
   *
   * DO_NOTHING FIRST - extra pixels added to the first relative component LAST - extra pixels added
   * to the last relative component LARGEST (default) - extra pixels added to the larger relative
   * component EQUAL - a single pixel is added to each relative component (until pixels are used up)
   */
   ...
   * <p>
   * The preferred width of a column layout is the largest preferred width of each column in the
   * container, plus the horizontal padding times the number of columns minus one, plus the left and
   * right insets of the target container.
   * <p>
   * The preferred height of a column layout is the largest preferred height of each row in the
   * container, plus the vertical padding times the number of rows minus one, plus the top and
   ...
   * <p>
   * The minimum width of a grid layout is the largest minimum width of each column in the
   * container, plus the horizontal padding times the number of columns minus one, plus the left and
   * right insets of the target container.
   * <p>
   * The minimum height of a column layout is the largest minimum height of each row in the
   * container, plus the vertical padding times the number of rows minus one, plus the top and
   * bottom insets of the target container.
   *
   * @param target the container in which to do the layout
   ...
   * constraints of the <code>RelativeLayout</code> object.
   * <p>
  ...
  /*
   * Because of rounding, all the space has not been allocated Override this method to create a
  ...
  /*
   * Largest component using relative sizing gets all the space. When multiple components are the
   * same size, the last one found is used.
   */
  ...
  /*
   * Each component using relative sizing gets 1 more pixel until all the space is used, starting
   * with the first.
   */
