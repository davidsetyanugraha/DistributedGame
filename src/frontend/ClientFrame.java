...
public class ClientFrame {
	...
	private void initialize() {
		...
		gridBagLayout.columnWeights = new double[] { 1.0D, 0.0D, 0.0D, 0.0D, 0.0D, 0.0D, 0.0D, 0.0D, 0.0D, 1.0D, 0.0D };
		gridBagLayout.rowWeights = new double[] { 0.0D, 0.0D, 0.0D, 0.0D, 1.0D, 0.0D, 0.0D, 0.0D, 0.0D, 1.0D };
	...
	/**
	 * start new game public void newMenuClicked(MouseEvent e) { Object[] options =
	 * { "Yes", "No" }; int decision = JOptionPane.showOptionDialog(this.frmClient,
	 * "Do you want to create a new game", "Tips", 0, 3, null, options, options[0]);
	 * if (decision == 0) { this.backPanel.reset(); this.backPanel.regame(); if
	 * (this.serverThread != null) { SocketMsg msg = new SocketMsg();
	 ...
	 * this.fileChooser.setFileFilter(new
	 * FileFilter(FileFilter.CanvasFilterType.COF_OPEN)); if
	 * (this.fileChooser.showOpenDialog(this.frmClient) == 0) { File f =
	 * this.fileChooser.getSelectedFile(); if (f.exists()) { try { ObjectInputStream
	 * in = new ObjectInputStream(new FileInputStream(f.getPath()));
	 * List<CanvasShape> sh = (List)in.readObject(); in.close();
	 * this.canvasPanel.reset(); this.canvasPanel.repaint();
	 * this.canvasPanel.setShapes((CopyOnWriteArrayList)sh);
	 * this.canvasPanel.repaint(); this.openedFile =
	 * this.fileChooser.getSelectedFile(); if (this.serverThread != null) {
	 * SocketMsg msg = new SocketMsg(); msg.setOperation("newCanvas");
	 * this.canvasPanel.sendMsg(msg); } for (CanvasShape shape : sh) { SocketMsg msg
	 * = new SocketMsg(); msg.setOperation("add"); JSONSerializer serializer = new
	 * JSONSerializer(); String data = serializer.serialize(shape);
	 * msg.setData(data); this.canvasPanel.sendMsg(msg); } } catch
	 * (FileNotFoundException ex) { ex.printStackTrace(); } catch (IOException ex) {
	 * ex.printStackTrace(); } catch (ClassNotFoundException ex) {
	 * ex.printStackTrace(); } } } this.fileChooser.resetChoosableFileFilters(); }
	 */

	/**
	 * save game public void saveMenuClicked(MouseEvent e) { if (this.openedFile ==
	 * null) { this.fileChooser.setAcceptAllFileFilterUsed(false);
	 * this.fileChooser.setFileFilter(new
	 * CanvasFileFilter(CanvasFileFilter.CanvasFilterType.COF_SAVE)); if
	 * (this.fileChooser.showSaveDialog(this.frmClient) == 0) { File f =
	 * this.fileChooser.getSelectedFile(); if (f.exists()) { showFileExistDlg(f); }
	 * else { this.userChoice = 1; } if (this.userChoice == 1) { try {
	 * ObjectOutputStream out = new ObjectOutputStream(new
	 * FileOutputStream(f.getPath()));
	 * out.writeObject(this.canvasPanel.getShapes()); out.close(); } catch
	 * (FileNotFoundException ex) { ex.printStackTrace(); } catch (IOException ex) {
	 * ex.printStackTrace(); } } this.userChoice = -1; } } else { try {
	 * ObjectOutputStream out = new ObjectOutputStream(new
	 * FileOutputStream(this.openedFile.getPath()));
	 * out.writeObject(this.canvasPanel.getShapes()); out.close(); } catch
	 * (FileNotFoundException ex) { ex.printStackTrace(); } catch (IOException ex) {
	 * ex.printStackTrace(); } } this.fileChooser.resetChoosableFileFilters(); }
	 */
	...
	/**
	 * save as public void saveAsMenuClicked(MouseEvent e) { CanvasFileFilter filter
	 * = new CanvasFileFilter(CanvasFileFilter.CanvasFilterType.SAVE);
	 * this.fileChooser.setAcceptAllFileFilterUsed(false);
	 * this.fileChooser.setFileFilter(filter); if
	 * (this.fileChooser.showSaveDialog(this.frmClient) == 0) { try { File f =
	 * this.fileChooser.getSelectedFile(); if (f.exists()) { showFileExistDlg(f); }
	 * else { this.userChoice = 1; } if (this.userChoice == 1) { String ext =
	 * filter.getExtension(f.getPath()); BufferedImage img =
	 * this.canvasPanel.getImage(); ImageIO.write(img, ext, f); } } catch
	 * (IOException ex) { ex.printStackTrace(); } this.userChoice = -1; }
	...
	public void showFileExistDlg(File f) {
		double xpos = this.frmClient.getLocationOnScreen().getX() + this.frmClient.getSize().getWidth() / 2.0D
				- this.popUpDialog.getSize().getWidth() / 2.0D;
		double ypos = this.frmClient.getLocationOnScreen().getY() + this.frmClient.getSize().getHeight() / 2.0D
				- this.popUpDialog.getSize().getHeight() / 2.0D;
		this.popUpDialog.setLocation((int) xpos, (int) ypos);
		this.popUpMsg.setText("File : \"" + f.getName() + "\" already exists.\nDo you want to overwrite?");
	 ...
	 * connect function
	 * 
	 * public void conNetClicked(MouseEvent e) { double x_pos =
	 * (this.frmClient.getBounds().getMinX() + this.frmClient.getBounds().getMaxX())
	 * / 7.0D * 2.0D; double y_pos = (this.frmClient.getBounds().getMinY() +
	 * this.frmClient.getBounds().getMaxY()) / 5.0D * 2.0D;
	 * this.conNetFrame.setLocation((int)x_pos, (int)y_pos);
	 * this.conNetFrame.setVisible(true); }
	 */

	/**
	 * end connection function
	 *
	 * public void endNetClicked(MouseEvent e) { if ((this.serverThread != null) ||
	 * (this.clientThread != null)) { Object[] options = { "end anyway", "nope" };
	 * int decision = JOptionPane.showOptionDialog(this.frmClient, "would you like
	 * to disconnect", "disconnect from network", 0, 3, null, options, options[0]);
	 * if (decision == 0) { if (this.serverThread != null) { SocketMsg msg = new
	 * SocketMsg(); msg.setOperation("disconnect"); msg.setData("the manager has
	 * disconnected"); this.canvasPanel.sendMsg(msg);
	 * 
	 * CopyOnWriteArrayList<User> clients = this.serverThread.getClients(); try {
	 * for (User client : clients) { if (client.getClientSocket() != null) {
	 * client.getClientSocket().close(); } }
	 * this.serverThread.getCanvasServer().close(); this.serverThread = null;
	 * this.user = null; } catch (IOException ioe) {
	 * System.out.println(ioe.getMessage()); } clients.removeAll(clients); } else if
	 * (this.clientThread != null) { try { this.clientThread.getClient().close();
	 * this.clientThread = null; getMenuBar().getMenu(0).setEnabled(true); } catch
	 * (IOException ioe) { System.out.println(ioe.getMessage()); } }
	 * getUserListPanel().clear(); } } else {
	 ...
	 * 
	 * public void setServerThread(SocketThread serverThread) { this.serverThread =
	 * serverThread; }
	 * 
	 * public SocketClient getClientThread() { return this.clientThread; }
	 * 
	 * public void setClientThread(SocketClient clientThread) { this.clientThread =
	 * clientThread; }
	 */
	...
	/**
	 * 
	 * public void sendMsgClicked() { if ((this.serverThread == null) &&
	 * (this.clientThread == null)) { JOptionPane.showMessageDialog(this.frmClient,
	 * "please start or connect to a network first", "error", 0); } else if
	 * (!this.tfMsg.getText().trim().equals("")) { String msgContent =
	 * this.tfMsg.getText(); SocketMsg msg = new SocketMsg();
	 * msg.setOperation("chat"); msg.setData(msgContent);
	 * msg.setUserName(getUser().getUserName()); JSONSerializer serializer = new
	 * JSONSerializer(); String msgJson = serializer.serialize(msg); if
	 * (this.serverThread != null) { CopyOnWriteArrayList<User> clients =
	 * getServerThread().getClients(); if (!clients.isEmpty()) { int i = 0; for
	 * (User client : clients) { if (i == 0) { i++; } else { try { DataOutputStream
	 * out = new DataOutputStream(client.getClientSocket().getOutputStream());
	 * out.writeUTF(msgJson); } catch (IOException e) {
	 * System.out.println(e.getMessage()); } } } } } else if (this.clientThread !=
	 * null) { try { DataOutputStream out = new
	 * DataOutputStream(getClientThread().getClient().getOutputStream());
	 * out.writeUTF(msgJson); } catch (IOException e) {
