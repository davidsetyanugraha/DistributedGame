    ...
    this.userList.addMouseListener(new MouseAdapter()
    {
      public void mouseClicked(MouseEvent mevt)
      {
        ...
        if (mevt.getClickCount() == 2)
        {
          ...
          if ((index > 0) && 
            (UserListPanel.this.parentFrm.getServerThread() != null))
          {
            Object[] options = { "Yes", "No" };
            int decision = JOptionPane.showOptionDialog(UserListPanel.this.parentFrm.getFrmClient(), 
              "Do you want to kick out " + (String)UserListPanel.this.userArray.get(index), "Tips", 0, 
              3, null, options, options[0]);
            if (decision == 0)
            {
              CopyOnWriteArrayList<User> clients = UserListPanel.this.parentFrm.getServerThread().getClients();
              for (User client : clients) {
                if (((String)UserListPanel.this.userArray.get(index)).equals(client.getUserName()))
                {
                  ...
                  UserListPanel.this.parentFrm.getUserListPanel().deleteUser((String)UserListPanel.this.userArray.get(index));
